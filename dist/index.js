...
function getArticleSummariesForAI() {
  return helpArticles.filter((article) => !article.externalLink).map((article) => `- "${article.title}" (${article.category}): ${article.excerpt}`).join("\n");
}
...
var appRouter = router({
  ...
  support: router({
    askAgent: publicProcedure.input(z2.object({ question: z2.string().min(1).max(1e3) })).mutation(async ({ input }) => {
      const { question } = input;
      const articleSummaries = getArticleSummariesForAI();
      const relevantArticles = helpArticles.filter((article) => !article.externalLink).filter((article) => {
